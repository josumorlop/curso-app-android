    ...
    fun loadCalendar() {
        ...
        repository.getCalendar(idClient, object : CalendarRepository.CalendarCallback {
            override fun onSuccess(calendar: List<CalendarDTO>) {
                ...
                _reportsLD.value = calendar.map { CalendarOne(it.id, it.cliente, it.usuarioRegistro, it.checkin, it.checkout, it.fecha, it.hora, it.horaFin, it.comentarios, it.comentarios2) }
                _pendingsLD.value = 4
            }
